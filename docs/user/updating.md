...
A graceful server restart, including a deploy that stops and starts the service,
resumes supported threads that were actively working. A crash or forced kill
does not.
...
ready. Providers with native promptless continuation use it; other providers
receive a short instruction to continue where they left off. Terminal commands
