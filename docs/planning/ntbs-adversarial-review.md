...
### 1. Outcome detection is anchored on "latest turn", which is wrong the moment anyone touches the thread
...
### 2. No timeout anywhere — a silently hung provider leaves an ack dangling forever
...
### 3. The event subscription cannot survive a restart, and nothing compensates
...
### 4. Double-posting is possible and undocumented
...
### 5. The turn-start bootstrap already exists as a single command — do not rebuild it from Git primitives
...
### 6. Provenance is first-class in T3 and the skeleton cannot carry it
...
### 7. `T3Context` is missing everything else a thread needs, with no stated defaulting policy
...
### 10. `platform-handler.ts` is a vacuous abstraction — delete it
...
### 11. Collapse `ProcessorEvent` — the union wraps two statically-known callers
...
### 12. Justify each of the five states with a distinct recovery action, or cut to three
...
### 13. Drop the tag factories until something resolves them from context
...
### 14. Design the error taxonomy around retryability, not strings
...
### 15. Do not collapse the outcome to `text` before the adapter sees it
...
### 16. Naming/file nits, worth fixing while it is cheap
