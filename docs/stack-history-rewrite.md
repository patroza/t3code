...
Same habit outside full restacks: when repairing `fork/changes` or an overlay tip with stack push
bypass, **amend/rewrite** the bad commit instead of leaving tip-only recovery commits. If a
permanent overlay draft PR was closed during the rewrite, **reopen that PR number** (after the
branch is fixed); do not mint a replacement unless reopen fails — then label **`OVERLAY`** and
update `pr-stack.json`. See [fork-stack.md](./fork-stack.md) (“Permanent draft PRs”, “Fixing layer
tips”).
