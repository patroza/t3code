...
| `recency`       | Group by recency  | Recency sections       | Latest-activity order with recency headers |
...
must not select a different row component or visual treatment. Recency headers may partition the
active block and the expanded Settled shelf; pinned cards stay above those buckets.
