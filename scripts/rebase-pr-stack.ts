...
export function verifyReplayHead(
  ...
  const vpBin = NodePath.join(repoDir, "node_modules", ".bin", "vp");
  const vp = NodeFS.existsSync(vpBin) ? vpBin : `${vpBin}.cmd`;
  console.log(`verify-head: ${sha} ${subject} → ${packages.join(", ")}`);
  for (const pkg of packages) {
    // Invoke Vite+ directly instead of `pnpm exec`: this preserves each
    // workspace's real typecheck script without triggering an implicit install.
    const result = run(vp, ["run", "--filter", pkg, "typecheck"], {
      cwd: repoDir,
      allowFailure: true,
      ...gitOpts,
    });
