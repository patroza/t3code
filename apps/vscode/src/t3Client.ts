...
export class T3Client {
  ...
  #buildRpcSession(prepared: PreparedConnection): Effect.Effect<RpcSession, never, never> {
    ...
    return Effect.gen(function* () {
      const connected = yield* Deferred.make<void>() as any;
      const disconnected = yield* Deferred.make<never, unknown>() as any;
