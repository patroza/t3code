...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const archiveThreadById = async (threadId: ThreadId): Promise<void> => {
    await ensureConnected();
    await client.archiveThread(threadId);
    const replacement = client.threadsForWorktree(worktreePath())[0];
    if (replacement !== undefined) {
      await client.selectThread(replacement.id);
      await client.waitForActiveThread();
      await rememberThread(replacement.id);
    } else {
      await context.workspaceState.update(activeThreadStorageKey(), undefined);
    }
  };
