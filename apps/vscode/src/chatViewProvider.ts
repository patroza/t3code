...
type WebviewRequest =
  | {
      readonly type: "ready" | "refresh" | "stop" | "toggleContext" | "archiveThread" | "visitT3";
    }
  | {
  ...
  | { readonly type: "hostResourceLive"; readonly live: boolean }
  | {
...
function isRequest(value: unknown): value is WebviewRequest {
  ...
  if (type === "hostResourceLive") {
    return "live" in value && typeof value.live === "boolean";
  }
  if (type === "selectModel") {
    return (
      "instanceId" in value &&
...
export class T3ChatViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  #hostResourcesLive = false;
  #hostResourceTimer: ReturnType<typeof setInterval> | null = null;
  ...
  constructor(client: T3Client, actions: ChatViewActions, extensionUri: vscode.Uri) {
    ...
    this.#disposables = [
      ...
      client.onVcsStatusChanged(() => this.#publish()),
    ...
    try {
      switch (message.type) {
        ...
        case "refresh":
          ...
          this.#syncHostResourcePolling();
          return;
        case "hostResourceLive":
          if (this.#hostResourcesLive === message.live) return;
          this.#hostResourcesLive = message.live;
          this.#syncHostResourcePolling();
          return;
        case "visitT3": {
        ...
        case "archiveThread": {
          ...
          if (thread.session?.status === "running") {
            throw new Error("Stop the running thread before archiving it.");
          }
          await this.#run(() => this.actions.archiveThread(thread.id));
          return;
        }
        case "sendNewThread":
  ...
  #watchWorktreeVcsStatus(): void {
    try {
      this.client.watchVcsStatus(this.actions.worktreePath());
    } catch {
      // No workspace folder is open, so there is no checkout to track.
    }
  }

  /**
   * Host metrics poll on a slow heartbeat and step up to a live cadence while the
   * pointer is over the gauge, mirroring the web client. Polling is suspended
   * while the view is hidden so a collapsed panel costs nothing.
   */
  #syncHostResourcePolling(): void {
    this.#stopHostResourcePolling();
    if (this.#view?.visible !== true) return;
    void this.#pollHostResources();
    this.#hostResourceTimer = setInterval(
      () => void this.#pollHostResources(),
      this.#hostResourcesLive ? HOST_RESOURCE_LIVE_INTERVAL_MS : HOST_RESOURCE_IDLE_INTERVAL_MS,
    );
  }

  #stopHostResourcePolling(): void {
    if (this.#hostResourceTimer === null) return;
    clearInterval(this.#hostResourceTimer);
    this.#hostResourceTimer = null;
  }

  async #pollHostResources(): Promise<void> {
    try {
      // A null snapshot means "disconnected" and hides the gauge, so stale numbers
      // never outlive the connection they came from.
      const snapshot = await this.client.hostResourceSnapshot();
      // Published on its own channel rather than through `#publish` so a 1s hover
      // cadence never re-renders the conversation.
      await this.#view?.webview.postMessage({ type: "hostResources", snapshot });
    } catch {
      // Host metrics are advisory; a failed probe leaves the last values on screen.
    }
  }
  ...
  #publish(): void {
    ...
    const vcsStatus = this.client.vcsStatus;
    ...
    void view.webview.postMessage({
      ...
      state: {
        ...
        changeRequest: resolveChangeRequestIndicator(
          vcsStatus?.pr,
  ...
  #serializeThread(thread: OrchestrationThread | null) {
    ...
    return {
      ...
      // Without this the composer cannot read back the thread's option choices
      // and every re-render resets each control to its descriptor default.
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* The column needs an explicit 0 minimum: a grid item's default min-width:auto
       is its min-content width, so one long \`pre\` line (white-space: pre) would
       otherwise widen the whole panel and push every row past the viewport. */
    #app { height: 100%; display: grid; grid-template-rows: auto 1fr auto; grid-template-columns: minmax(0, 1fr); }
    .toolbar { display: grid; grid-template-columns: minmax(0, 1fr) auto auto auto; gap: 6px; padding: 10px; border-bottom: 1px solid var(--vscode-sideBar-border); }
    ...
    .toolbar-menu-popup { position: absolute; top: calc(100% + 4px); right: 0; z-index: 30; min-width: 130px; border: 1px solid var(--vscode-menu-border, var(--vscode-editorWidget-border)); border-radius: 5px; padding: 4px; background: var(--vscode-menu-background, var(--vscode-editorWidget-background)); box-shadow: 0 4px 14px var(--vscode-widget-shadow); }
    .toolbar-menu-popup button { width: 100%; border: 0; padding: 5px 8px; text-align: left; background: transparent; color: var(--vscode-menu-foreground, var(--vscode-foreground)); }
    .toolbar-menu-popup button:hover:not(:disabled) { background: var(--vscode-menu-selectionBackground, var(--vscode-list-activeSelectionBackground)); color: var(--vscode-menu-selectionForeground, var(--vscode-list-activeSelectionForeground)); }
    .toolbar-menu-separator { margin: 4px 2px; border: 0; border-top: 1px solid var(--vscode-menu-separatorBackground, var(--vscode-editorWidget-border)); }
    ...
    .messages-shell { position: relative; min-height: 0; }
    #messages { height: 100%; min-height: 0; overflow-y: auto; padding: 12px 10px 18px; display: flex; flex-direction: column; gap: 14px; }
    #new-activity { position: absolute; bottom: 8px; left: 50%; z-index: 2; display: inline-flex; align-items: center; gap: 6px; transform: translateX(-50%); border-color: var(--vscode-sideBar-border); border-radius: 999px; box-shadow: 0 2px 8px rgb(0 0 0 / 18%); white-space: nowrap; }
    #new-activity[hidden] { display: none; }
    .new-activity-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--vscode-charts-blue); }
    ...
    .context-trailing { display: flex; align-items: center; gap: 5px; margin-left: auto; }
    #change-request { display: inline-flex; align-items: center; gap: 3px; border: 0; padding: 3px 5px; background: transparent; color: var(--vscode-descriptionForeground); font-size: 10px; font-variant-numeric: tabular-nums; white-space: nowrap; }
    #change-request:hover { background: color-mix(in srgb, var(--vscode-descriptionForeground) 10%, transparent); text-decoration: underline; }
    #change-request[hidden] { display: none; }
    #change-request.open { color: var(--vscode-charts-green); }
    #change-request.merged { color: var(--vscode-charts-purple, #a970ff); }
    #change-request svg { display: block; }
    .host-resources-control { position: relative; }
    #host-resources-toggle { display: inline-flex; align-items: center; gap: 4px; border: 0; padding: 3px 5px; background: transparent; color: var(--vscode-descriptionForeground); font-family: var(--vscode-editor-font-family); font-size: 10px; font-variant-numeric: tabular-nums; white-space: nowrap; cursor: default; }
    #host-resources-toggle[hidden] { display: none; }
    .host-metric { display: inline-flex; align-items: center; gap: 3px; }
    .host-metric + .host-metric { margin-left: 4px; }
    .host-metric-meter { position: relative; width: 3px; height: 10px; overflow: hidden; border-radius: 2px; background: color-mix(in srgb, var(--vscode-descriptionForeground) 25%, transparent); }
    .host-metric-fill { position: absolute; right: 0; bottom: 0; left: 0; border-radius: inherit; background: var(--vscode-descriptionForeground); }
    .host-metric.warning { color: var(--vscode-charts-orange); }
    .host-metric.warning .host-metric-fill { background: var(--vscode-charts-orange); }
    .host-metric.critical { color: var(--vscode-charts-red); }
    .host-metric.critical .host-metric-fill { background: var(--vscode-charts-red); }
    .host-resources-details { position: fixed; z-index: 20; display: none; grid-template-columns: auto minmax(0, 1fr); gap: 3px 10px; border: 1px solid var(--vscode-editorWidget-border); border-radius: 7px; padding: 8px; background: var(--vscode-editorWidget-background); box-shadow: 0 5px 22px var(--vscode-widget-shadow); color: var(--vscode-foreground); font-size: 10px; }
    .host-resources-control:hover .host-resources-details { display: grid; }
    .host-resources-detail-label { color: var(--vscode-descriptionForeground); }
    .host-resources-detail-value { white-space: nowrap; }
    #context-window { --context-percent: 0deg; position: relative; min-width: 30px; height: 22px; padding: 0 4px; border-radius: 11px; background: conic-gradient(var(--vscode-charts-blue) var(--context-percent), color-mix(in srgb, var(--vscode-descriptionForeground) 25%, transparent) 0); color: var(--vscode-foreground); font-size: 9px; font-weight: 600; }
