      ...
      return yield* Option.match(result, {
        onNone: () =>
          Effect.gen(function* () {
            ...
            // the authoritative route forever: every later tool call is queued
            // to the same dead stream and pays another full timeout. Completing
            // the stream makes the client subscription reconnect and lets the
            // replacement host register with a fresh connection id. Do not
