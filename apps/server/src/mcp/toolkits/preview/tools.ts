...
export const PreviewSnapshotTool = readonlyBrowserTool(
  Tool.make("preview_snapshot", {
    description:
      "Inspect a page before interacting. Pass tabId to inspect a specific tab; omit it to use this agent session's current tab. Returns page state, semantic elements, diagnostics, action history, a PNG screenshot, and a local artifact path when the host supports export. Set includeImage=false for text-only output with the same page metadata. Set save=true to also write the PNG to disk and get screenshotPath back. To show the user a screenshot, embed the exported path or screenshotPath as ![alt](path); the image in the tool result is not saved anywhere unless save=true or the host already exported a path.",
