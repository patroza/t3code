...
function providersFor(value: "all" | "codex" | "claude" | "opencode"): ReadonlyArray<Provider> {
  switch (value) {
    case "codex":
      return ["codex"];
    case "claude":
...
export const importSessionsCommand = Command.make("import-sessions", {
  ...
  Command.withHandler((flags) =>
    Effect.sync(() => {
      const baseDir = homePath(
        Option.getOrUndefined(flags.baseDir) ?? process.env.T3CODE_HOME ?? "~/.t3",
      );
      const dbPath = NodePath.join(baseDir, "userdata", "state.sqlite");
      const cwd = normalizeCwd(Option.getOrUndefined(flags.cwd));
      const sessionId = Option.getOrUndefined(flags.sessionId);
      const scanInput = {
        limit: flags.limit,
        ...(sessionId !== undefined ? { sessionId } : {}),
        ...(cwd !== undefined ? { cwd } : {}),
      };
      const sessions = providersFor(flags.provider).flatMap((provider) => {
        switch (provider) {
          case "codex":
      ...
      const results = sessions.map((session) => ({
        ...
        title: session.title,
        cwd: session.cwd,
        status: flags.dryRun ? "dry-run" : importSession(dbPath, baseDir, session),
      }));
      if (flags.json) {
