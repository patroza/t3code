  ...
  return Effect.gen(function* () {
          ...
          const startedExit = yield* Effect.exit(
            Effect.gen(function* () {
              ...
              const resolved = yield* Effect.gen(function* () {
                ...
                const adoptedDirectory = adopted
                  ? readOpenCodeSessionDirectory(adopted)
                  : undefined;
                const reusable =
                  adopted &&
                  (adoptedDirectory === undefined ||
                    adoptedDirectory === normalizeDirectory(directory))
                    ? adopted
                    : undefined;
    ...
    const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
      ...
      return {
        threadId: input.threadId,
        turnId,
        // Echo the durable binding so ProviderService keeps
        // provider_session_runtime.resume_cursor_json fresh on every turn, not
        // just at session start (mirrors the other resumable providers).
