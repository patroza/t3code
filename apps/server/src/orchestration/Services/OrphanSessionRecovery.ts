...
export interface OrphanSessionRecoveryShape {
  ...
  readonly settleThread: (input: {
    ...
    /**
     * Preferred terminal status when work was in progress.
     * Zombie live sessions with no active turn always settle to `ready`
     * (never Wake Required) regardless of this preference.
     */
    readonly status?: Extract<OrchestrationSession["status"], "interrupted" | "stopped" | "ready">;
  }) => Effect.Effect<void>;
