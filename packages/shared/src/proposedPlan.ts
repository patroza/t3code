/**
 * Pure proposed-plan helpers shared by web and VS Code.
 * Keep free of DOM / React so both clients can reuse the same semantics.
 */

export interface ProposedPlanFields {
  readonly id: string;
...
export function findLatestProposedPlan<T extends ProposedPlanFields>(
...
export function hasActionableProposedPlan(
  proposedPlan: Pick<ProposedPlanFields, "implementedAt"> | null,
): boolean {
 ...
 * exists in plan mode — not only after the agent turn settles.
 */
export function shouldShowPlanFollowUpComposer(input: {
  readonly interactionMode: string | undefined | null;
  readonly hasPendingUserInput: boolean;
  readonly proposedPlan: Pick<ProposedPlanFields, "implementedAt"> | null;
}): boolean {
  return (
...
/** Status pill: plan ready outranks Working when a plan is actionable. */
